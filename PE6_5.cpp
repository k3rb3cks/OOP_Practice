...
int main()
{
    ...
    {   
        ...
        p[i] = Inventory(author, title, publisher, price, stocks);
        std::cout<<std::endl;
    ...
    do
    {
        ...
        for(int i = 0; i < 5; i++)
        {
            found = Search_book(author, title, &p[i]);
            if(found)
            {
                p->successful++;
                break;
    ...
    std::cout<<"Successfull Transactions: "<<p->successful<<std::endl;
    std::cout<<"Unsuccessfull Transactions: "<<p->unsuccessful<<std::endl;
